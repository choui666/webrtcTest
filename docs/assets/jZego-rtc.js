...
Object.defineProperty(exports, "__esModule", { value: true });
var pitchUtil = /** @class */ (function () {
    function pitchUtil(context) {
        this.delayTime = 0.100;
        this.fadeTime = 0.05;
        this.startTime = 0.100;
        ...
        this.shiftDownBuffer = this.createDelayTimeBuffer(context, this.startTime, this.fadeTime, false);
        this.shiftUpBuffer = this.createDelayTimeBuffer(context, this.startTime, this.fadeTime, true);
        ...
        this.fadeBuffer = this.createFadeBuffer(context, this.startTime, this.fadeTime);
        ...
        var t2 = t + this.startTime - this.fadeTime;
        this.mod1.start(t);
        this.mod2.start(t2);
        this.mod3.start(t);
        this.mod4.start(t2);
    ...
    pitchUtil.prototype.createFadeBuffer = function (context, startTime, fadeTime) {
        var length1 = startTime * context.sampleRate;
        var length2 = (startTime - 2 * fadeTime) * context.sampleRate;
        var lenght = length1 + length2;
        var buffer = context.createBuffer(1, lenght, context.sampleRate);
        var p = buffer.getChannelData(0);
        var fadeIndex1 = fadeTime * context.sampleRate;
        var fadeIndex2 = length1 - fadeIndex1;
        for (var i = 0; i < length1; ++i) {
            if (i < fadeIndex1) {
                p[i] = Math.sqrt(i / fadeIndex1);
            }
            else if (i >= fadeIndex2) {
                p[i] = Math.sqrt(1 - (i - fadeIndex2) / fadeIndex1);
            }
            else {
                p[i] = 1;
            }
    ...
    pitchUtil.prototype.createDelayTimeBuffer = function (context, startTime, fadeTime, shiftUp) {
        var length1 = startTime * context.sampleRate;
        var length2 = (startTime - 2 * fadeTime) * context.sampleRate;
    ...
    pitchUtil.prototype.setDelay = function (delayTime) {
        this.modGain1.gain.setTargetAtTime(0.5 * delayTime, 0, 0.010);
        this.modGain2.gain.setTargetAtTime(0.5 * delayTime, 0, 0.010);
    };
    pitchUtil.prototype.setPitchOffset = function (mult) {
        if (mult > 0) {
...
var ZegoClient = /** @class */ (function (_super) {
    ...
    */
    ZegoClient.prototype.startPublishingStream = function (streamid, localVideo, extraInfo, playOption) {
        var _this = this;
        this.logger.info('zc.p.sps.1 call streamid: ' + streamid);
        if (!streamid) {
    ...
    ZegoClient.prototype.stopPublishingStream = function (streamid) {
        this.logger.info('zc.p.sps.1.1 call streamid: ' + streamid);
        if (!streamid) {
    ...
    ZegoClient.prototype.startScreenSharing = function (screenConfig, audio, callBack) {
        var _this = this;
        if ('getDisplayMedia' in navigator.mediaDevices) {
            var zegoMediaDevices = navigator.mediaDevices;
            zegoMediaDevices.getDisplayMedia({
                audio: audio, video: {
                    frameRate: screenConfig.frameRate || 15,
                    //@ts-ignore
                    displaySurface: screenConfig.displaySurface || 'minitor'
                },
            })
    ...
    ZegoClient.prototype.startScreenShotFirFox = function (screenConfig, mediaSource, audio, callBack) {
        var _this = this;
        var config = {
            video: {
                ...
                frameRate: screenConfig.frameRate || 15,
                bitRate: screenConfig.bitRate || 800
            },
            audio: audio
        };
    ...
    ZegoClient.prototype.voiceChange = function (mult, streamid) {
        ...
        var publisher = this.getPublisher(streamid);
        return publisher.voiceChange(mult);
    };
    ZegoClient.prototype.voiceBack = function (streamid) {
        var publisher = this.getPublisher(streamid);
        return publisher.voiceBack();
    };
...
var ZegoPlayWeb = /** @class */ (function () {
    function ZegoPlayWeb(log, signal, dataReport, qualityTimeInterval, streamCenter) {
    ...
    ZegoPlayWeb.prototype.startPlay = function (streamId, remoteVideo, audioOutput, playOption) {
        ...
        this.negoTimer = setTimeout(function () {
            if (_this.stateNego !== zego_entity_1.ENUM_PLAY_STATE_NEGO.iceConnected && _this.negoTryCount < _this.negoTryMaxCount) {
                _this.signal.sendCloseSession(zego_extern_1.getSeq(), _this.sessionId, 0);
                _this.resetPlay();
                _this.startPlay(streamId, remoteVideo, audioOutput, playOption);
                ++_this.negoTryCount;
            }
            else if (_this.stateNego !== zego_entity_1.ENUM_PLAY_STATE_NEGO.iceConnected && _this.negoTryCount === _this.negoTryMaxCount) {
                _this.logger.error('zp.sp.1 waiting timeout');
                _this.playStateUpdateError(zego_extern_1.playErrorList.SERVER_NEGO_TIMEOUT);
            }
        }, this.negoInterval);
    };
    ;
    ...
    ZegoPlayWeb.prototype.onCreatePlaySessionSuccess = function (data) {
        ...
        this.signal.registerPushCallback('StreamStatusNotifyPush', this.sessionId, function (seq, sessionId, data) {
            _this.onRecvStreamStatus(seq, sessionId, data);
        });
    ...
    ZegoPlayWeb.prototype.onSetLocalDescriptionSuccess = function (desc) {
        ...
        this.signal.sendMediaDesc(this.answerSeq, this.sessionId, 0, mediaDescription, function (seq, sessionId, data) {
            _this.logger.info('zp.osd.1 sendMediaDesc resp');
            ...
            _this.logger.info('zp.osd.1 send success stateNego:waiterAnswer');
    ...
    ZegoPlayWeb.prototype.onRecvMediaDesc = function (seq, sessionId, data) {
        ...
        this.logger.info('zp.orm.1 received stateNego:waitingCandidate');
    ...
    ZegoPlayWeb.prototype.onRecvCandidateInfo = function (seq, sessionId, data) {
        var _this = this;
        this.logger.info('zp.orci.1 received ');
    ...
    ZegoPlayWeb.prototype.onIceConnectionStateChange = function (event) {
        ...
        if (this.peerConnection.iceConnectionState === 'connected') {
            ...
            for (var i in this.streamCenter.publisherList) {
                if (this.streamCenter.publisherList[i].publisher.state == zego_entity_1.ENUM_PUBLISH_STATE.publishing) {
                    this.signal.sendBroadcasterStatus(zego_extern_1.getSeq(), this.sessionId, 1);
                    break;
                }
            }
            ...
            this.logger.info('zp.oisc.1  stateNego:iceConnected');
            this.negoTryCount = 1;
            if (this.negoTimer) {
    ...
    */
    ...
    ZegoPlayWeb.prototype.uploadPlayQuality = function (playData) {
        ...
        if (this.qualityUploadLastTime == 0 || timeStamp - this.qualityUploadLastTime >= this.qualityUploadInterval) {
            ...
            this.logger.info('zp.upq.1 upload' + JSON.stringify(playData));
    ...
    ZegoPlayWeb.prototype.onRecvCloseSession = function (seq, sessionId, data) {
        ...
        // let reason = data.reason * 1;
        //
        // // 如果收到服务返回错误，清楚掉客户端超时重试逻辑；只调用错误码重试逻辑
        // this.negoTimer && clearTimeout (this.negoTimer);
        // if (typeof reason === 'number' && [4, 8, 10, 11, 12, 26, 27].includes (reason) && (this.negoTryCount < this.negoTryMaxCount)) {
        //         this.logger.info ('zp.orcs.1 retry: ' + this.streamId);
        //         let streamId = this.streamId;
        //         let remoteVideo = this.remoteVideo;
        //         let audioOutput = this.audioOutput;
        //         let playOption = this.playOption;
        //         this.signal.sendCloseSession (getSeq (), this.sessionId, 0);
        //         this.resetPlay ();
        //         this.startPlay (streamId, remoteVideo, audioOutput, playOption);
        //         ++this.negoTryCount;
     ...
     */
    ZegoPlayWeb.prototype.onRecvStreamStatus = function (seq, sessionId, data) {
        this.logger.debug('zp.orss.0 call');
        if (sessionId != this.sessionId) {
            this.logger.error('zp.orss.0 cannot find session');
            return;
        }
        var stream = this.remoteVideo.srcObject;
        if (data.camera_status == 2 && stream.getVideoTracks().length !== 0) {
        ...
        if (data.camera_status == 0 && stream.getVideoTracks().length == 0) {
            this.remoteVideo.srcObject = this.remoteStream.clone();
        }
        this.logger.debug('zp.orss.0 call success');
    };
    //////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ZegoPlayWeb.prototype.onGotRemoteStream = function (stream) {
        ...
        this.remoteVideo.srcObject = stream.clone();
    ...
    ZegoPlayWeb.prototype.sendCandidateInfo = function (candidateInfo) {
        var _this = this;
        this.logger.info('zp.sci.1 called');
        ...
        this.logger.info('zp.sci.1  stateNego:sendCandidate');
    ...
    ZegoPlayWeb.prototype.playStateUpdateError = function (errorCode) {
        this.logger.info('zp.psue.1 called ', errorCode.code);
        if (this.state === zego_entity_1.ENUM_PLAY_STATE.stop || (this.negoTryCount < this.negoTryMaxCount && this.stateNego < zego_entity_1.ENUM_PLAY_STATE_NEGO.iceConnected)) {
            //already reset
            return;
        }
    ...
    ZegoPlayWeb.prototype.stopPlay = function () {
        this.logger.info('zp.sp.1.1 called');
        ...
        for (var j in this.streamCenter.playerList) {
            if (this.streamCenter.playerList[j].player.state = zego_entity_1.ENUM_PLAY_STATE.playing) {
                isPlay = true;
                break;
            }
        }
        (isPublish && !isPlay) && this.signal && this.signal.sendBroadcasterStatus(zego_extern_1.getSeq(), this.sessionId, 0);
        //send to server
...
var ZegoPreview = /** @class */ (function () {
        ...
        this.enableCamera = function (enable, streamCenter) {
            ...
            for (var i in streamCenter.publisherList) {
                ...
                publisher.localStream == _this.localStream &&
                    publisher.signal.sendStreamStatus(zego_extern_1.getSeq(), publisher.sessionId, enable ? 0 : 2, 0);
            }
    ...
    ZegoPreview.prototype.startPreview = function (localVideo, mediaStreamConfig, successCallback, errorCallback) {
        ...
        if (mediaStreamConfig.externalMediaStream instanceof MediaStream) {
            // let videoinfo = mediaStreamConfig.externalMediaStream.getVideoTracks ()[0].getSettings ();
            ...
            if (result) {
                ...
                if (errorCallback) {
                    errorCallback('external capture fail');
                }
            }
            return;
        }
        ...
        navigator.mediaDevices.getUserMedia(mediaStreamConstraints).then(function (stream) {
            ...
            _this.logger.info('zp.sv.2 failed ' + error.message);
            if (errorCallback) {
    ...
    ZegoPreview.prototype.captureStream = function (localVideo) {
        ...
        if (localVideo['captureStream']) {
            ...
            this.logger.info('zp.cs.2 don\'t support capture stream');
            return false;
        }
...
var audioMixUtil_1 = __webpack_require__(/*! ../util/audioMixUtil */ "./sdk/util/audioMixUtil.ts");
var sdpUtil_1 = __webpack_require__(/*! ../util/sdpUtil */ "./sdk/util/sdpUtil.ts");
var pitchUtil_1 = __webpack_require__(/*! ../util/pitchUtil */ "./sdk/util/pitchUtil.ts");
var ZegoPublish = /** @class */ (function () {
    function ZegoPublish(log, signal, dataReport, qualityTimeInterval, streamCenter) {
    ...
    ZegoPublish.prototype.publishStateUpdateError = function (errorCode) {
        this.logger.error('zp.psu.0 call ' + JSON.stringify(errorCode));
        if (this.state === zego_entity_1.ENUM_PUBLISH_STATE.stop || (this.negoTryCount < this.negoTryMaxCount && this.stateNego < zego_entity_1.ENUM_PUBLISH_STATE_NEGO.iceConnected)) {
            //already reset
            return;
        }
        if (this.sessionId != 0 && this.shouldSendCloseSession(errorCode)) {
   ...
   */
    ZegoPublish.prototype.startPublish = function (streamId, localStream, videoInfo, mediaStreamConfig, playOption) {
        var _this = this;
        ...
        this.playOption = playOption || {};
        // firefox循环播放第三方视频时触发
        if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1 && (mediaStreamConfig.externalCapture || mediaStreamConfig.externalMediaStream)) {
            this.localStream.onaddtrack = function () {
                _this.logger.info('zp.sp.0 Track added');
                var videoTracks = _this.localStream.getVideoTracks();
                var audioTracks = _this.localStream.getAudioTracks();
                if (videoTracks.length > 1) {
                    var sender = _this.peerConnection.getSenders().find(function (s) { return s.track.kind === videoTracks[1].kind; });
                    sender.replaceTrack(videoTracks[1]);
                    _this.localStream.removeTrack(videoTracks[0]);
                }
                else if (audioTracks.length > 1) {
                    var sender = _this.peerConnection.getSenders().find(function (s) { return s.track.kind === audioTracks[1].kind; });
        ...
        this.logger.info('zp.sp.0 called success');
        ...
        this.negoTimer = setTimeout(function () {
            if (_this.stateNego !== zego_entity_1.ENUM_PUBLISH_STATE_NEGO.iceConnected && _this.negoTryCount < _this.negoTryMaxCount) {
                _this.signal.sendCloseSession(zego_extern_1.getSeq(), _this.sessionId, 0);
                _this.resetPublish();
                _this.startPublish(streamId, localStream, videoInfo, mediaStreamConfig, playOption);
                ++_this.negoTryCount;
            }
            else if (_this.stateNego !== zego_entity_1.ENUM_PUBLISH_STATE_NEGO.iceConnected && _this.negoTryCount === _this.negoTryMaxCount) {
                _this.logger.error('zp.sp.0 waiting timeout');
                _this.publishStateUpdateError(zego_extern_1.publishErrorList.SERVER_NEGO_TIMEOUT);
            }
        }, this.negoInterval);
    };
    ;
    ...
    ZegoPublish.prototype.onSetLocalDescriptionSuccess = function (desc) {
        ...
        this.signal.sendMediaDesc(this.offerSeq, this.sessionId, 0, mediaDescription, function (seq, sessionId, data) {
            ...
            _this.logger.info('zp.osd.0 send success stateNego:waiterAnswer');
    ...
    ZegoPublish.prototype.onRecvMediaDescription = function (seq, sessionId, data) {
        ...
        this.logger.info('zp.orm.0 received stateNego:waitingCandidate');
        //clear timer
        if (this.waitingAnswerTimer != null) {
            clearTimeout(this.waitingAnswerTimer);
            this.waitingAnswerTimer = null;
    ...
    */
    ZegoPublish.prototype.onIceConnectionStateChange = function (event) {
        ...
        if (this.peerConnection.iceConnectionState === 'connected') {
            ...
            for (var i in this.streamCenter.playerList) {
                if (this.streamCenter.playerList[i].player.state == zego_entity_1.ENUM_PLAY_STATE.playing) {
                    this.signal.sendBroadcasterStatus(zego_extern_1.getSeq(), this.sessionId, 1);
                    break;
                }
            }
            ...
            this.logger.info('zp.oisc.0  stateNego:iceConnected');
            this.negoTryCount = 1;
            if (this.negoTimer) {
                clearTimeout(this.negoTimer);
            }
    ...
    ZegoPublish.prototype.sendCandidateInfo = function (candidateInfo) {
        ...
        this.logger.info('zp.sci.0  stateNego:sendCandidate');
        this.signal.sendCandidateInfo(zego_extern_1.getSeq(), this.sessionId, candidateInfo, function (seq, sessionId, data) {
            _this.logger.info('zp.sci.0 send success');
            _this.dataReport.eventEnd(_this.reportSeq, 'SendIceCandidate');
        }, function (err, seq) {
    ...
    ZegoPublish.prototype.onRecvCandidateInfo = function (seq, sessionId, data) {
        var _this = this;
        this.logger.info('zp.oci.0 received ' + JSON.stringify(data.infos));
    ...
    ZegoPublish.prototype.onRecvCloseSession = function (seq, sessionId, data) {
        ...
        //         let mediaStreamConfig = this.mediaStreamConfig;
        //         let playOption = this.playOption;
        //         this.signal.sendCloseSession (getSeq (), this.sessionId, 0);
        //         this.resetPublish ();
    ...
    ZegoPublish.prototype.tryStartPublish = function (streamId) {
        ...
        this.currentRetryCount += 1;
        if (this.signal.isServerConnected()) {
            this.logger.info('zp.tsp.0 signal connected');
            this.startPublish(streamId, this.localStream, this.videoInfo, this.mediaStreamConfig);
        }
        else {
            //setTimer
    ...
    ZegoPublish.prototype.setPublishQualityTimer = function () {
        ...
        this.logger.info('zp.spq.0 called');
        this.clearPublishQualityTimer();
        this.qualityTimer = setInterval(function () {
            if (_this.peerConnection) {
                _this.peerConnection.getStats(null).then(function (results) {
    ...
    ZegoPublish.prototype.uploadPublishQuality = function (publishData) {
        ...
        if (this.qualityUploadLastTime == 0 || timeStamp - this.qualityUploadLastTime >= this.qualityUploadInterval) {
            ...
            this.logger.info('zp.upq.0 upload' + JSON.stringify(publishData));
    ...
    */
    ZegoPublish.prototype.stopPublish = function () {
        this.logger.info('zp.sp.0.1 called');
        //sendBroadcasterStatusNotify
        var isPublish = false;
        var isPlay = false;
        for (var i in this.streamCenter.publisherList) {
    ...
    //voice change
    ZegoPublish.prototype.voiceChange = function (mult) {
        var context = new (typeof webkitAudioContext !== 'undefined' ? webkitAudioContext : AudioContext)();
        var streamSource = null;
        var destination = null;
        if (!this.pitchEffect) {
            this.pitchEffect = new pitchUtil_1.pitchUtil(context);
            streamSource = context.createMediaStreamSource(this.localStream.clone());
        ...
        if (!this.micTrack) {
            var audioTrack_1 = destination.stream.getAudioTracks()[0];
            var sender = this.peerConnection.getSenders().find(function (s) { return s.track.kind === audioTrack_1.kind; });
            if (!sender) {
            ...
            this.localStream.removeTrack(this.micTrack);
            this.localStream.addTrack(audioTrack_1);
        }
    };
    ZegoPublish.prototype.voiceBack = function () {
        var _this = this;
        if (!this.micTrack) {
...
var ZegoStreamCenterWeb = /** @class */ (function (_super) {
    ...
    function ZegoStreamCenterWeb(log, stateCenter) {
        ...
        _this.signalList = {};
        _this.tryCountConnectInterval = 9000;
        _this.maxTryCountConnect = 3;
    ...
    ZegoStreamCenterWeb.prototype.switchDevice = function (type, localVideo, deviceId, success, error) {
        ...
        preview = this.checkPreview(localVideo);
        if (!preview) {
        ...
        if (type === 'video') {
            ...
            lastTrack = localVideo.srcObject.getVideoTracks()[0];
        }
        else {
            mediaStreamConfig.audioInput = deviceId;
            lastTrack = localVideo.srcObject.getAudioTracks()[0];
        }
        ...
        navigator.mediaDevices.getUserMedia(mediaStreamConstraints).then(function (stream) {
            ...
            if (streamId) {
                publisher = _this.publisherList[streamId].publisher;
            ...
            publisher.signal.sendStreamStatus(zego_extern_1.getSeq(), publisher.sessionId, preview.localStream.getVideoTracks()[0].enabled ? 0 : 2, preview.localStream.getAudioTracks()[0].enabled ? 0 : 2);
            success && success();
        }, function (err) { return error && error(err); });
    };
    /*
    ...
    ZegoStreamCenterWeb.prototype.setPublishStateStart = function (streamid, localVideo, playOption) {
        ...
        var publisher = new zego_publish_1.ZegoPublish(this.logger, null, this.dataReport, this.qualityTimerInterval, this);
   ...
   */
    ...
    ZegoStreamCenterWeb.prototype.startPublishingStream = function (streamid, serverUrls, preferPublishSourceType) {
        ...
        var serverUrl = this.server || serverUrls[0];
        publish.serverUrls = publish.serverUrls.concat(serverUrls).sort();
    ...
    ZegoStreamCenterWeb.prototype.publishStream = function (streamid) {
        ...
        publisher.startPublish(streamid, localStream, videoInfo, preview.mediaStreamConfig, playOption);
    };
    ZegoStreamCenterWeb.prototype.connectPublishServer = function (streamId, serverUrl) {
        ...
        this.connetWithReuseSignalServerTimer(streamId, true, function (streamid, signalInfo) {
            //check streamid exist
    ...
    ZegoStreamCenterWeb.prototype.stopPublishingStream = function (streamid) {
        ...
        this.publisherList[streamid].countConnectTimer && clearTimeout(this.publisherList[streamid].countConnectTimer);
    ...
    ZegoStreamCenterWeb.prototype.setPlayStateStart = function (streamid, remoteVideo, audioOutput, playOption) {
        ...
        var player = new zego_play_web_1.ZegoPlayWeb(this.logger, null, this.dataReport, this.qualityTimerInterval, this);
    ...
    ZegoStreamCenterWeb.prototype.startPlayingStream = function (streamid, serverUrls, currentPlaySourceType) {
        ...
        play.serverUrls = play.serverUrls.concat(serverUrls).sort();
        return this.connectPlayServer(totalStreamId, this.server || serverUrls[0]);
    };
    ;
    /*
    ...
    ZegoStreamCenterWeb.prototype.connectPlayServer = function (streamId, serverUrl) {
        ...
        this.connetWithReuseSignalServerTimer(streamId, false, function (streamid, signalInfo) {
            //check streamid exist
     ...
     */
    ZegoStreamCenterWeb.prototype.connetWithReuseSignalServerTimer = function (streamId, isPublish, success, error) {
        var _this = this;
        var serverUrls, serverUrl;
        if (isPublish) {
            serverUrls = this.publisherList[streamId].serverUrls;
            if (this.publisherList[streamId].tryCountConnect > this.maxTryCountConnect) {
                this.logger.error('zs.crsst.0 beyond max limit');
                return;
            }
            this.logger.info('zs.crsst.0 called ' + this.publisherList[streamId].tryCountConnect);
            this.publisherList[streamId].countConnectTimer = setTimeout(function () {
                _this.connetWithReuseSignalServerTimer(streamId, isPublish, success, error);
            ...
            }, this.publisherList[streamId].tryCountConnect === this.maxTryCountConnect ? error : undefined);
            ++this.publisherList[streamId].tryCountConnect;
        }
        else {
            serverUrls = this.playerList[streamId].serverUrls;
            if (this.playerList[streamId].tryCountConnect > this.maxTryCountConnect) {
                this.logger.error('zs.crsst.0 beyond max limit');
                return;
            }
            ...
            }, this.playerList[streamId].tryCountConnect === this.maxTryCountConnect ? error : undefined);
    ...
    ZegoStreamCenterWeb.prototype.stopPlayingStream = function (streamid) {
        ...
        this.playerList[totalStreamId].countConnectTimer && clearTimeout(this.playerList[totalStreamId].countConnectTimer);
